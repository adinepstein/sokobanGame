 ...
 *
 * @author Adin Epstein
 * @since 23/12/2016 The class loads a text file
 *
 */
		...
		while ((line = bufferedReader.readLine()) != null) {
			...
			for (int j = 0; j < line.length(); j++) {
				...
				case '#':
					...
					level.getMap().get(i).add(wall);
					break;
				case '@':
					Box box= new Box(i,j);
					level.getMap().get(i).add(box);
					level.getBoxList().add(box);
					break;
				case ' ':
					level.getMap().get(i).add(new Floor(i,j));
					break;
				case 'o':
					Target target=new Target(i,j);
					level.getMap().get(i).add(target);
					level.getTargetList().add(target);
					level.addTarget();
					break;
				case 'A':
